...
def check_user_spam(user_id):
    if GLOBAL_USER_DATA.get("AntiSpam") and GLOBAL_USER_DATA["AntiSpam"].get(user_id):
        status = GLOBAL_USER_DATA["AntiSpam"].get(user_id).get("status")
    else:
        status = False
    if GLOBAL_USER_DATA.get("AntiSpamHard") and GLOBAL_USER_DATA["AntiSpamHard"].get(
        user_id
    ):
